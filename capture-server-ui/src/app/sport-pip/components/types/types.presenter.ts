...
@Component({
    ...
    template: `<app-types [datasource]="ds" (save)="actions.onSave()" (cancel)="actions.onCancel()"></app-types>`,
...
export class TypesPresenter  {
    ...
    public set key(v : string) {
        if(v != this._key){
            this._key = v;
            Transformer.ComposeObjectAsync(this.dataFactory.MetaTypeByKey(v), this.ds, TypesBuilder);
        }
    }  
    ...
    setModalActions(onClose: EventEmitter<any>): void {
        this.actions = new ModalActions(this.ds, this.dataFactory.SaveMetaType, TypesEgressBuilder, onClose)
    }
