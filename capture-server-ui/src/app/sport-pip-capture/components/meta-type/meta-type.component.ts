...
export class MetaTypeComponent implements OnInit {
  ...
  backgroundColors: { [key: number]: string } = {};
   metaType: { id: number; values: string[] | undefined; key: string, name: string }
  constructor(private el: ElementRef, private dataFactory: DataFactoryService) {

  }


  // private _metaType: { id: number; values: string[] | undefined; key: string, name: string };
  // public get metaType(): { id: number; values: string[] | undefined; key: string, name: string } {
  //   if (!this._metaType) {
  //     this.metaType = this.datasource
  //     console.log(this._metaType)
  //   }
  //   return this._metaType;
  // }
  // public set metaType(v: { id: number; values: string[] | undefined; key: string, name: string }) {
  //   this._metaType = v;
  // }
  ...
  OnChangeType(i: number, item: { id: number; values: string[]; key: string; name: string; }) {
    this.newValue = '';
    this.datasource.metatype.forEach((element: { backgroundColor: string; color: string; }) => {
  ...
  UpdateRow(item: string) {
    ...
    if (index !== -1) {
      ...
      this.dataFactory.update('meta-type', data, this.metaType.id).subscribe(d => {
        console.log(d)
      })
