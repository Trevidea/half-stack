...
export class Collection<T> extends Range<T> {
  ...
  private _LatentSelectionPredicate: (
    value: T,
    index: number | null,
    obj: T[] | null
  ) => unknown = null;
  ...
  private _setSelection(item: T): void {
    if (this.SelectedItem === null && this._LatentSelectionItem === item) {
      ...
      this._LatentSelectionPredicate &&
      this._LatentSelectionPredicate(item, 0, [])
    ) {
  ...
  public set SelectedItem(v: T) {
    ...
    if (v === null) {
      ...
      if (idx >= 0) {
        ...
        this.itemSelectedDispatcher.fire({ selectedItem: this._SelectedItem });
      } else {
  ...
  public set SelectedIndex(v: number) {
    ...
    if (v >= 0 && v < this.length) {
      ...
      this.SelectedItem = super[this._SelectedIndex];
    }
  }
